...
namespace HSRPWebServices
{
    ...
    public class HSRPService : System.Web.Services.WebService
    {
                    ...
                    else
                    {
                        if (HSRP_StateID == "4")
                        {
                            if (MobileNo.Length > 0)
                            {
                                ...
                                string sqlQuery = "select roundoff_netamount from hsrprecords where VehicleRegNo='" + VehicleRegNo + "'";


                                amount = utils.getDataSingleValue(sqlQuery, CnnString, "roundoff_netamount");
                                string SMSText = "Cash Rs. " + amount + "  received against HSRP " + VehicleRegNo + " on " + System.DateTime.Now + " receipt number " + CashReceiptNo + ". Transport Department Haryana.";
                                ...
                                //string sendURL = "http://103.16.101.52:8080/bulksms/bulksms?username=sse-hsrphr&password=hsrphr&type=0&dlr=1&destination=9810509118&source=HRHSRP&message=" + SMSText;
                                ...
                                utils.ExecNonQuery("insert into HRSMSDetail(RtoLocationID,VehicleRegNo,MobileNo,SentResponseCode,SmsText) values('" + RTOLocationID + "','" + VehicleRegNo + "'," + MobileNo + ",'" + result + "','" + SMSText + "')", CnnString);
                            }
                        }  
        ...
        public String CheckDuplicateEntry(string HSRPStateID, string AuthNo, string VehicleRegNo, string OrderType)
        {
            ...
            string sqlText = "select COUNT(VehicleRegNo) as co from dbo.HSRPRecords  where hsrp_stateid ='" + HSRPStateID + "' and VehicleRegNo ='" + VehicleRegNo.Replace(" ", "") + "' and OrderType='" + strOrderType + "' and HSRPRecord_AuthorizationNo='" + AuthNo.Replace(" ", "") + "'";
        ...
        {
            string CnnString = obj.strProvider;
            string SqlQuery = "select VehicleRegNo,Vehicletype,ChassisNo,EngineNo,OrderType,HsrpRecord_AuthorizationNo,hsrp_front_lasercode,hsrp_rear_lasercode from hsrprecords where hsrp_stateid='" + HsrpStateID + "' and VehicleRegNo='" + VehicleRegno + "' and orderstatus='New Order'";
        ...
        public string CheckLaserInHsrpRecords(string strOrederType, string strFrontLaser, string strRearlaser)
        {
            ...
            if (strOrederType != "DR")
            {
                SqlQuery = "Select vehicleregno from hsrprecords where hsrp_stateid=3 and erpassigndate is null and hsrp_front_lasercode='" + strFrontLaser + "' or hsrp_rear_lasercode='" + strFrontLaser + "'";
                dtResult = utils.GetDataTable(SqlQuery, CnnString);
                if (dtResult.Rows.Count > 0)
                {
                    Message = "FrontUsed";
            ...
            {
                SqlQuery = "select vehicleregno from hsrprecords where hsrp_stateid=3 and erpassigndate is null and hsrp_front_lasercode='" + strRearlaser + "' or hsrp_rear_lasercode='" + strRearlaser + "'";
                dtResult = utils.GetDataTable(SqlQuery, CnnString);
                if (dtResult.Rows.Count > 0)
                {
                    Message = "RearUsed";
        ...
        //#region Send Record For Updation to Specific RTO
        //[WebMethod(Description = "Send Record For Updation")]
        //public DataTable GetDataForSticker(string strRto)
        //{            
        //    DataTable dtResult = new DataTable();         
        //    string CnnString = obj.strProvider;
        //    string strSqlQuery = string.Empty;
        //    strSqlQuery = "select * from HP_PortabilityData where ";
        //    dtResult = utils.GetDataTable(strSqlQuery, CnnString);
        //    dtResult.TableName = "HP_Portability";
        //    return dtResult;
        //}
